...
class Arbitrum(Ethereum):
    ...
    def gethost(self):
        host = environ.get("ARBITRUM_API_SERVER_HOST", "arbitrum-shkeeper")
        port = environ.get("ARBITRUM_SERVER_PORT", "6000")
        return f"{host}:{port}"

    def get_auth_creds(self):
        username = environ.get(f"ARB_USERNAME", "shkeeper")
        password = environ.get(f"ARB_PASSWORD", "shkeeper")
