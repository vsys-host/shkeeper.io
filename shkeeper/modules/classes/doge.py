...
class Doge(Crypto):
    ...
    def gethost(self):
        host = environ.get("DOGE_API_SERVER_HOST", "dogecoin-shkeeper")
        port = environ.get("DOGE_SERVER_PORT", "6000")
        return f"{host}:{port}"

    def get_auth_creds(self):
        username = environ.get(f"DOGE_USERNAME", "shkeeper")
        password = environ.get(f"DOGE_PASSWORD", "shkeeper")
