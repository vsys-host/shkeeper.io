...
class Monero(Crypto):
    ...
    def create_wallet(self):
        try:
            self.monero_rpc_wallet.raw_request('create_wallet', {"filename": self.MONERO_WALLET_NAME,
                                                                 "password": self.MONERO_WALLET_PASS,
                                                                 "language": "English"})
        except RPCError as exception:
            if 'Cannot create wallet. Already exists.' not in str(exception):
                raise exception
        self.monero_rpc_wallet.raw_request('open_wallet', {"filename": self.MONERO_WALLET_NAME,
                                                           "password": self.MONERO_WALLET_PASS})
        return {'error': None}
    ...
    def mkpayout(self, destination: str, amount: Decimal, fee: int, subtract_fee_from_amount: bool = False):
        try:
            priority_class = fee if fee in self.fixed_fee_steps else self.default_fee
            result = error = None
            if amount == self.balance():
                res_list = self.monero_wallet.sweep_all(destination, priority=priority_class)
                result = [res[0].hash for res in res_list]
            else:
                txs = self.monero_wallet.transfer(destination, amount, priority=priority_class)
    ...
    def estimate_tx_fee(self, amount, priority_class=None):
        ...
        fee =  self.monero_wallet.transfer(self.monero_wallet.addresses()[0],
                                           amount,
                                           priority=priority_class,
                                           relay=False)[0].fee
        app.logger.debug(f'Estimated fee for transfer of {amount} XMR with priority {priority_class} is {fee}')
        return fee
