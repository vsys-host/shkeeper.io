...
By default, the same API key is used as the **HMAC secret** when you [verify webhook callbacks](#webhook-signature-verification) from SHKeeper (`X-Shkeeper-Signature` / `X-Shkeeper-Timestamp`). You can configure a separate webhook HMAC secret in **Wallets → Manage → Payment Gateway**. When a separate secret is configured, API requests continue to use the API key while outbound webhooks are signed with the dedicated secret.
...
For new integrations, **verify `X-Shkeeper-Signature`** using the dedicated webhook HMAC secret configured in SHKeeper. If no dedicated secret is configured, use the API key for backward compatibility. Do not rely on `X-Shkeeper-Api-Key` alone. Full algorithm, examples (Python, Flask, PHP), and security notes: **[Webhook signature verification (HMAC-SHA256)](https://github.com/vsys-host/shkeeper-docs/blob/main/docs/getting-started/webhook_verification.md)**.
...
```text
HMAC_SHA256(webhook_secret_or_api_key, "{timestamp}." + raw_request_body)
```
...
- `webhook_secret_or_api_key` — the dedicated webhook secret when configured; otherwise the API key.
...
3. Recompute HMAC with your dedicated webhook secret, or the API key when using the compatibility fallback, and compare signatures (constant-time).
