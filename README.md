...
SHKeeper supports user authentication and authorization through an API key generated by the user. Send it in the `X-Shkeeper-Api-Key` HTTP header: `X-Shkeeper-Api-Key: <API_KEY>`.

Keep the two credentials below separate in new integrations:

| Receiver configuration name | Purpose |
|---|---|
| `API_KEY` | Authenticates API requests that your application sends to SHKeeper. |
| `WEBHOOK_SECRET` | Verifies HMAC signatures on callbacks that SHKeeper sends to your application. It is not an API credential. |

By default, SHKeeper uses the API key as the webhook signing key for backward compatibility. You can activate a dedicated `WEBHOOK_SECRET` in **Wallets > Manage > Payment Gateway**. Activating it does not change `API_KEY`: API requests continue to use the API key, while outbound callbacks use only the dedicated webhook secret.
...
| `X-Shkeeper-Api-Key` | Invoice callbacks in compatibility mode only. Payout callbacks never include this legacy header. |

The active signing mode determines both the HMAC key and whether invoice callbacks include the legacy API-key header:

| Signing mode | HMAC key stored by your callback receiver | Invoice `X-Shkeeper-Api-Key` header |
|---|---|---|
| Dedicated secret (recommended) | `WEBHOOK_SECRET` | Omitted |
| Compatibility fallback | `API_KEY` | Included |

Payout callbacks never send `X-Shkeeper-Api-Key`, regardless of signing mode. When a dedicated secret is active, no callback sends the API key.

For new integrations, **verify `X-Shkeeper-Signature` with a dedicated `WEBHOOK_SECRET`**. Do not trust a credential supplied by the callback itself; load the expected secret from your receiver's secure configuration.

To activate a dedicated secret safely:

1. Open **Wallets > Manage > Payment Gateway** and select **Generate**, or enter your own secret containing at least 32 characters.
2. Generation creates a candidate only. It does not save it or change the key currently signing callbacks.
3. Copy the exact candidate into the callback receiver's `WEBHOOK_SECRET` configuration and deploy that configuration.
4. Return to SHKeeper and select **Activate**. Confirming activation immediately switches every wallet to the new secret. The stored value is not shown again.
5. To return to compatibility mode, select **Use API key**. SHKeeper then signs callbacks with `API_KEY`; invoice callbacks also resume sending the legacy `X-Shkeeper-Api-Key` header.

For a zero-downtime rotation, temporarily let the receiver verify both the old and candidate secrets. Remove the old secret after activation and after any in-flight callback retries have completed.

When replacing an active secret, configure the receiver with the candidate before confirming replacement. Otherwise, the receiver will reject callbacks signed after the switch. When adapting an example from the linked guide, pass `WEBHOOK_SECRET` as its `secret` in dedicated mode; `API_KEY` is correct only in compatibility mode. Algorithm, examples (Python, Flask, PHP), and security notes: **[Webhook signature verification (HMAC-SHA256)](https://github.com/vsys-host/shkeeper-docs/blob/main/docs/getting-started/webhook_verification.md)**.
...
```text
signing_key = WEBHOOK_SECRET if a dedicated secret is active, otherwise API_KEY
signature = HMAC_SHA256(signing_key, "{timestamp}." + raw_request_body)
```
...
- `WEBHOOK_SECRET` — the dedicated callback-verification secret copied into your receiver before activation.
- `API_KEY` — the API credential, used for signature verification only while compatibility fallback is active.
...
3. Recompute HMAC with the locally configured `WEBHOOK_SECRET`, or the locally configured `API_KEY` when using compatibility fallback, and compare signatures in constant time.
...
**Documentation:** verification algorithm and language examples — [webhook_verification.md](https://github.com/vsys-host/shkeeper-docs/blob/main/docs/getting-started/webhook_verification.md). Use `WEBHOOK_SECRET` as the example's `secret` when dedicated mode is active.
